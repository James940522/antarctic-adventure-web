...
export function GameHud({ run, onRestart }: { run: GameSnapshot; onRestart: () => void }) {
  ...
  const { next, message } = run.landmarks;
  return <div className="pointer-events-none absolute inset-0 text-slate-950">
    <div className="absolute inset-x-0 top-0 grid grid-cols-[1fr_1fr_1fr] items-start gap-[1%] bg-gradient-to-b from-slate-950/85 to-slate-950/0 px-[3%] pb-[5%] pt-[2%] text-white">
      ...
      <div aria-label="다음 랜드마크" className="min-w-0 text-center leading-snug">
        <p className="text-[clamp(8px,1.3cqw,12px)] font-bold tracking-[0.2em] text-cyan-200">NEXT</p>
        <p className="mt-[0.3cqw] break-keep text-[clamp(10px,1.7cqw,16px)] font-bold">{next?.name ?? "--"}</p>
        {next && <p className="text-[clamp(10px,1.7cqw,16px)] tabular-nums text-cyan-100">{next.distanceRemaining.toLocaleString()} m</p>}
      </div>
    ...
    {!gameover && !run.paused && message && <div role="status" aria-label="랜드마크 통과" className="absolute inset-x-[22%] top-[19%] text-center text-slate-900">
      <p className="text-[clamp(11px,2cqw,20px)] font-black tracking-widest">{formatDistance(message.distance).toUpperCase()}</p>
      <p className="text-[clamp(12px,2.5cqw,24px)] font-bold">{message.name}</p>
      <p className="text-[clamp(8px,1.2cqw,12px)] font-semibold tracking-[0.15em]">{message.nameEn.toUpperCase()}</p>
    </div>}
