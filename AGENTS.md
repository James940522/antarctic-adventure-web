...
2026-09-23 최신 장애물 확장은 13종과 `1 | 2 | "full"` 실제 점유를 사용한다. 아래의 이전 6종·균등 빈도 규칙보다 `docs/OBSTACLE_ASSETS_PLAN.md`의 **현재 최종 사양**이 우선한다. 바리케이드는 단독 전체 레인 점유·점프 전용이며 13종의 그림/충돌/아이템 배치는 동일한 `occupiedLanes` 기반 인스턴스 형상을 사용한다. 점프 기능은 활성 상태다.

현재 기본 모드는 **남극 장애물 회피 거리 기록 모드**다. 최신 장애물 규칙은 `docs/OBSTACLE_ASSETS_PLAN.md`를 따르며, 이전 다중 네온 박스·거리별 밀도 증가 계획보다 우선한다.

- 앱 진입은 React 메인 메뉴다. 클래식 버전을 선택해야 Phaser가 생성되고, 활성 창에서 입력 없이 14m/s로 자동 전진을 시작한다. 기본 속도는 실제 주행 1초마다 0.01m/s씩 연속 증가한다(1분 후 14.6m/s, 5분 후 17m/s). 수동 일시정지·창/탭 이탈·랜드마크 도착 연출·게임오버 중에는 증가하지 않고 재도전하면 14m/s로 초기화한다. ↑/↓ 또는 스틱·D-pad·터치 버튼을 누르는 동안 초당 8m/s씩 연속 조절하며 가속 상한은 없다. 감속 하한은 시간에 따라 올라간 기본 속도이고, 입력을 놓으면 수동 가속분은 유지된다.
- 6종 남극 장애물이 실제 전진량에 맞춰 다가온다. 기존 7개 레인에서 덜 사용한 위치를 우선해 좌우 분포를 고르게 한다. 초반 2개, 2km 이후 최대 3개, 5km 이후 최대 4개를 묶어 각각 6~12m씩 앞뒤로 엇갈리게 배치한다. 종류는 6종 shuffle bag이며 각 사이클에 한 번씩 나온다. 묶음의 마지막 장애물부터 다음 묶음까지 35~55m, 2칸 장애물을 포함한 묶음 뒤에는 45~65m를 띄운다. 묶음마다 공통 회피 레인을 남기며 30m/s에서도 다음 회피 공간으로 이동 가능해야 한다. 첫 50m 동안 화면에 장애물이 없고 이후 지평선에서 등장한다.
...
현재 속도 모델은 `PlayerState.baseSpeed`, `selectedSpeed`, `currentSpeed`로 표현한다. 설정의 초기 `baseSpeed` 140은 14m/s이고 `manualAcceleration` 80 월드 단위/s²는 수동 가감속 초당 8m/s다. `baseAcceleration` 0.1 월드 단위/s²에 따라 실제 주행 중 기본 속도와 선택 속도가 함께 초당 0.01m/s씩 증가한다. 2026-09-24 사용자 요청에 따라 ↑/↓를 누르는 시간에 비례해 선택 속도를 초당 8m/s씩 연속 조절하고 상한은 두지 않는다. 감속은 현재 기본 속도까지만 가능하며 입력을 놓으면 수동 가속분을 유지한 채 완만하게 빨라진다. 거리 적분은 기본 증가와 수동 가감속을 함께 반영하며, 감속이 기본 속도에 닿는 프레임은 전후 구간을 나눠 적분한다. 충돌·아이템·도착의 거리 비율을 실제 시간 비율로 환산해 점프와 평균 속도 계산에 사용하며, 접촉 뒤 남은 시간의 증가량은 반영하지 않는다. 도착 중 현재 속도는 0이며 기본·선택 속도를 고정하고 재출발 때 선택 속도를 복원한다. 속도는 이동 거리, 장애물 접근, 배경 이동에 일관되게 반영한다. 이전의 새 입력마다 8m/s씩 즉시 변경하는 모델은 사용하지 않는다. Phaser의 기존 requestAnimationFrame 루프와 delta를 유지하며 별도 타이머를 추가하지 않는다. 상세 점검은 `docs/SPEED_CONTROL_REVIEW.md`를 참고한다.
...
- 1칸/2칸별 위치 사용 횟수를 관리해 덜 사용한 시작 칸을 우선하고 동률이면 무작위로 선택한다. 2칸은 마지막 레인에서 시작할 수 없다. 같은 묶음의 점유 칸은 겹치지 않고 최소 한 레인을 비운다. 넓은 장애물 때문에 빈 레인을 확보할 수 없으면 해당 종류를 다음 묶음으로 미룬다.
- `nextGroupDistance`는 월드 단위다. 첫 묶음은 50m 안전 구간 + 기존 120m 시야 거리 뒤에 놓여 50m 주행 후 화면에 등장한다. 초반 2개, 2km 이후 최대 3개, 5km 이후 최대 4개를 묶으며 각각 6~12m씩 엇갈린다. 마지막 장애물부터 다음 묶음까지 35~55m 간격, 직전 묶음에 2칸 장애물이 있으면 추가 10m를 적용한다. 30m/s·반응 여유 0.35초 기준으로 묶음 사이의 횡이동 경로도 확보한다.
- 6개 종류를 섞어 하나씩 소진하고 다시 채운다. 사이클 경계의 동일 종류 연속 출현을 막으며, 랜드마크 안전 구간과 겹치는 묶음은 종류와 위치 사용 횟수를 소진하지 않고 건너뛴다.
...
- CourseSegmentSystem, 물개 이동·공격, 피격 페널티 차등, 아이템 드롭은 이번 범위 밖이다.

## 10. 거리·평균 속도 점수와 후속 시간제 스테이지

현재 `RunSystem`은 RUNNING ↔ CELEBRATING, RUNNING → GAMEOVER와 전진 거리·평균 속도를 관리하고 `ScoreSystem`의 함수로 `round(정수 거리(m) × 평균 속도 / 14)`를 계산한다. 평균 속도는 반올림 전 이동 거리(m)를 실제 전진 시간(s)으로 나누며, 충돌 프레임은 접촉 시점까지의 시간만 포함한다. 랜드마크 정지·창 이탈 시간은 제외한다. 순간 선택 속도나 표시용 정수 playTime으로 평균을 재계산하지 않는다. 기본 14m/s에서 1km는 1,000점이며 거리와 평균 속도에 비례한다. 점수/속도 상한은 추가하지 않는다. 진행 중에는 평균 변화로 점수가 내려갈 수 있어 “예상 SCORE”로 표시하고 게임오버 때 확정한다. HUD·결과·랭킹의 거리는 정수 미터에 천 단위 쉼표를 넣어 표시하고 평균 속도는 소수 첫째 자리로 표시한다. 결과 문구는 “평균 … m/s로 … m 전진”이다.
...
5. **장애물:** 실제 PNG 6종, 거리별 2~4개 엇갈린 묶음, 균등한 1칸·2칸 위치, shuffle bag, 회피 공간, 즉사 충돌과 점프 회피.
