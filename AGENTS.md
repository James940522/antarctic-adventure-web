...
30m/s까지 점프 시간은 0.8초이고, 그보다 빠르면 `min(0.8초, 24m / 현재 속도)`로 짧아진다. 점프 높이는 100으로 유지한다. 공중 가감속은 진행도를 되돌리지 않고 남은 점프의 진행 속도에 반영한다. 고속에서 한 번의 점프가 차지하는 거리는 최대 24m로, 최소 행 간격 35m보다 짧게 유지한다. 프레임 내 포물선 궤적을 충돌 판정에 전달해 한 프레임 안의 이륙·정점·착지도 계산하며, 착지 뒤에는 새 입력으로 다음 점프를 시작한다.
