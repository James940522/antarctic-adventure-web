...
반응성, 가독성, 속도감이 시각적 복잡성보다 우선이다. 플레이 가능한 기본 루프를 먼저 완성한다. 곡선 코스, 비행 아이템, 고급 이펙트와 오디오는 후속 확장이다. 모바일·태블릿 터치 조작은 현재 지원 범위다.
...
터치 입력은 `TouchInput` 어댑터를 통해 같은 InputManager에 병합한다. 화면 폭 1024px 이하 또는 주 포인터가 coarse인 화면에 왼쪽·오른쪽 이동, 하단 중앙 점프, 작은 감속·가속 버튼을 표시한다. 버튼은 플레이 영역 아래에 두고 반투명 배경을 사용해 시야를 가리지 않는다. Pointer ID별로 held 상태를 관리하여 이동과 점프를 동시에 입력할 수 있게 한다. pointerup/cancel, 캡처 해제, 창 이탈, 화면 크기·방향 변경, 재시작·언마운트 시 잔류 입력을 정리한다.

모바일 화면 회전 기준은 **긴 변/짧은 변 ≥ 1.35**다. 태블릿 이하에서 세로가 긴 경우에만 게임·HUD·버튼을 함께 90도 회전한다. 가로 화면과 정사각형에 가까운 화면은 원래 방향을 유지한다. OS 방향 잠금에 의존하지 않고 CSS가 비율·회전을 소유한다. Phaser는 960×540 논리 크기를 유지하며 회전으로 인스턴스를 재생성하지 않는다.
...
현재 거리 기록 모드의 랜드마크는 `src/game/data/landmarks.ts`의 300m~10km 데이터로 관리한다. 사용자가 제공한 이미지 10개는 `public/asset/`의 원래 파일명·경로를 유지한다. `LandmarkSystem`은 충돌 판정 후 확정된 실제 거리를 미터로 받아, 코스 측면의 이미지 한 개와 다음 목표를 갱신한다. 랜드마크는 충돌·정지·난이도 변경이 없는 풍경이다. 기존 `PerspectiveSystem`을 재사용하며, 통과 메시지 시간은 주행의 시뮬레이션 시간을 따른다. React HUD에는 기존 100ms 스냅샷으로 전달한다. 재시작 시 전체 랜드마크 상태를 초기화하며, 10km 이후에도 달리기를 계속하고 NEXT는 `--`로 표시한다.
...
- 모바일·태블릿은 반투명 터치 버튼과 화면 비율에 따른 가로 배치를 제공한다. 데스크톱 키보드·게임패드 입력과 같은 모델을 사용한다.
