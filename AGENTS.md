...
```text
스테이지 시작 → 자동 전진 → 속도 조절 → 좌우 이동
→ 장애물 회피·점프 → 아이템 수집 → 남은 시간 관리
→ 목적지 기지 도착 → 스테이지 클리어 → 다음 스테이지
```
...
- 장애물 생성, 충돌, 점프 회피, 충돌 페널티
- 제한 시간, 이동 거리, 진행도, 기본 점수, HUD
- 스테이지 클리어, 시간 초과에 따른 게임 오버, 재시작
...
속도 모델은 `currentSpeed`, `minSpeed`, `maxSpeed`, `acceleration`, `deceleration`, `collisionSpeedLoss` 등으로 표현한다. ↑는 가속, ↓는 감속이며 자동 전진의 기본 속도와 최소 속도는 플레이 감각에 맞게 정한다. 속도는 이동 거리, 장애물 접근, 배경 이동, 난이도와 충돌 후 회복에 일관되게 반영한다.
...
초기 장애물은 `ICE_HOLE`, `CREVASSE`, `SEAL`이다. 한 종류로 생성·접근·회피·충돌을 먼저 완성하고 나머지를 추가한다. `ICE_BLOCK`, `ROCK`, `SNOW_MOUND`, `MOVING_ANIMAL` 등은 후속 확장이다.
...
- 일반 충돌은 즉사가 아니라 짧은 피격, 속도 감소, 일시적인 조작 제약 등으로 시간 손실을 유발한다. 직접적인 시간 차감이 필요하면 데이터로 명시한다.
...
5. **장애물:** 한 종류의 생성·접근·회수, 충돌과 점프 회피, 크레바스·물개 추가.
6. **플레이 루프:** 데이터 기반 단일 스테이지, 거리, 제한 시간, 기본 점수, HUD, 클리어·시간 초과·재시작.
...
- 스틱 드리프트가 걸러지고 입력 장치를 즉시 전환할 수 있다.
- 장애물이 지평선에서 접근하며 원근에 따라 커진다.
- 장애물 회피·점프·충돌 페널티가 플레이 결과에 반영된다.
- 제한 시간, 이동 거리, 기본 점수, HUD가 실제 게임 상태와 일치한다.
- 스테이지 클리어, 시간 초과 게임 오버, 깨끗한 재시작이 가능하다.
