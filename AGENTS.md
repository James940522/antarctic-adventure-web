...
터치 입력은 `TouchInput` 어댑터를 통해 같은 InputManager에 병합한다. 화면 폭 1024px 이하 또는 주 포인터가 coarse인 화면에 양 끝에 왼손·오른손 조작 그룹을 표시한다. 각 그룹은 위에 점프·속도 ↑·속도 ↓ 버튼을, 아래에 해당 방향의 큰 이동 버튼을 둔다. 동일 동작 버튼 두 개를 동시에 눌러도 한 입력으로 합친다. 버튼은 플레이 영역 아래에 두고 반투명 배경을 사용해 시야를 가리지 않는다. Pointer ID별로 held 상태를 관리하여 이동과 점프를 동시에 입력할 수 있게 한다. pointerup/cancel, 캡처 해제, 창 이탈, 화면 크기·방향 변경, 재시작·언마운트 시 잔류 입력을 정리한다.
...
현재 `RunSystem`은 RUNNING ↔ CELEBRATING, RUNNING → GAMEOVER와 실제 전진 거리 점수를 관리한다. `RecordStore`가 완료한 주행의 최고 정수 미터 기록을 해당 origin의 localStorage에 저장한다. 최고 기록 저장이 막히면 메모리에서 유지한다. 재시작은 거리·시간·속도(m/s)·점프·입력·장애물·랜드마크를 초기화하고 최고 기록만 보존한다. 아래 스테이지·타이머 규칙은 후속 별도 모드에만 적용한다.
...
현재 랜드마크 10종의 거리는 600/1,400/2,400/4,000/6,000/8,000/10,000/13,000/16,000/20,000m이며 `src/game/data/landmarks.ts`에서 관리한다. 사용자가 제공한 이미지는 `public/asset/`의 원래 파일명·경로를 유지한다. 기존 `PerspectiveSystem`으로 코스 정면에 하나씩 크게 표시하며, 300~700m에 걸쳐 접근한다. 접근 시작 20m 전부터 목적지 160m 뒤까지 박스 배치를 제외해 사전에 생성된 장애물도 연출을 가리지 않게 한다.

`RunSystem`이 실제 도착 거리에서 정확히 멈추고 `LandmarkSystem`의 축하 상태를 갱신한다. 2.5초 동안 거리·장애물·주행 시간은 고정하고, 마지막 0.35초에 이미지를 희미하게 지운 뒤 같은 선택 속도로 중앙에서 재출발한다. 축하 시간은 활성 창의 제한된 delta로만 흐르며 창/탭 이탈 때 함께 멈춘다. 진입·종료 시 입력을 초기화하고 축하 중 입력과 터치 버튼은 잠시 비활성화한다. React HUD는 기존 100ms 스냅샷으로 NEXT/ARRIVED를 표시한다. 같은 프레임의 도착 전 또는 동일 시점 충돌은 게임오버가 우선한다. 재시작 시 모든 도착 상태를 초기화하며, 최종 20km 도착 후에도 계속 달리고 NEXT는 `--`로 표시한다. 별도 CourseSegment 난이도는 이 도착 데이터에서 자동으로 파생하지 않는다.
