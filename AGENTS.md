...
현재 `RunSystem`은 RUNNING ↔ CELEBRATING, RUNNING → GAMEOVER와 실제 전진 거리 점수를 관리한다. 평균 속도는 반올림 전 이동 거리(m)를 실제 주행 시간(s)으로 나누며, 충돌 프레임은 접촉 시점까지의 시간만 포함한다. 랜드마크 정지·창 이탈 시간은 제외한다. 결과는 거리(km, 소수 셋째 자리)와 평균 속도(m/s, 소수 첫째 자리)를 함께 표시한다.
