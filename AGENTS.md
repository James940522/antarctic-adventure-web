...
- 앱 진입은 React 메인 메뉴다. 클래식 버전을 선택해야 Phaser가 생성되고, 활성 창에서 입력 없이 14m/s로 자동 전진을 시작한다. 기본 속도는 실제 주행 1초마다 0.01m/s씩 연속 증가한다(1분 후 14.6m/s, 5분 후 17m/s). 수동 일시정지·창/탭 이탈·랜드마크 도착 연출·게임오버 중에는 증가하지 않고 재도전하면 14m/s로 초기화한다. ↑/↓ 또는 스틱·D-pad를 새로 입력할 때 8m/s씩 조절하며 가속 상한은 없다. 감속 하한은 시간에 따라 올라간 기본 속도이고 누른 채로는 반복 가감속하지 않는다.
...
현재 속도 모델은 `PlayerState.baseSpeed`, `selectedSpeed`, `currentSpeed`로 표현한다. 설정의 초기 `baseSpeed` 140과 `speedStep` 80은 월드 단위이며 표시 속도로는 14m/s, 8m/s다. `baseAcceleration` 0.1 월드 단위/s²에 따라 실제 주행 중 기본 속도와 선택 속도가 함께 초당 0.01m/s씩 증가한다. ↑/↓를 새로 입력했을 때 선택 속도를 8m/s씩 조절하고 상한은 두지 않는다. 감속은 현재 기본 속도까지만 가능하며 입력을 놓으면 수동 가속분을 유지한 채 완만하게 빨라진다. 거리 적분에는 프레임 평균 속도를 사용하고, 충돌·도착 프레임의 증가량은 실제 진행 비율까지만 반영한다. 도착 중 현재 속도는 0이며 기본·선택 속도를 고정하고 재출발 때 선택 속도를 복원한다. 속도는 이동 거리, 장애물 접근, 배경 이동에 일관되게 반영한다. 이전의 버튼을 누른 채 연속 가감속·피격 감속 모델은 현재 모드에 적용하지 않는다.
